...
class HomeView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["all_category_posts"] = Post.objects.filter(
            published_at__isnull= False, status= "active"
        )[:6]
...
class ContactView(View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            form.save()
            messages.success(
            request, "Successfully submitted your query. We wil contact you soon"
            )
...
class PostSearchView(View):
    ...
    def get(self, request, *args, **kwargs):
        query = request.GET.get("query") #query = a search => title => a or content = a
        print(query)
